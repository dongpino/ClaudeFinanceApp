...
async function run() {
  // ── 1. 403 → 1회 재시도 → 200 XML: 아경 아이템이 결국 들어온다 ──
  {
    const attempts = { asiae: 0 };
    global.fetch = async (url) => {
      if (url.includes('yna.co.kr'))   return makeRes({ body: xmlFeed(['연합-정상기사']) });
      if (url.includes('edaily'))      return makeRes({ body: xmlFeed(['이데일리-정상']) });
      if (url.includes('coindesk'))    return makeRes({ body: xmlFeed(['coindesk-news']) });
      if (url.includes('asiae')) {
        attempts.asiae++;
        if (attempts.asiae === 1) return makeRes({ status: 403, contentType: 'text/html', body: 'blocked', retryAfter: '0' });
        return makeRes({ body: xmlFeed(['아경-재시도성공']) });
      }
      return makeRes({ status: 404, body: '' });
    };
    const items = await collectIssueSourceNews(30);
    assert(attempts.asiae === 2, '1: 아경 재시도 발생(2회 호출)');
    assert(has(items, '아경-재시도성공'), '1: 재시도 성공분이 결과에 포함');
    assert(has(items, '연합-정상기사'), '1: 정상 피드는 그대로');
  }

  // ── 2. 200-HTML(Cloudflare 챌린지): 아경 탈락, 나머지는 유지 ──
  {
    global.fetch = async (url) => {
      if (url.includes('yna.co.kr'))   return makeRes({ body: xmlFeed(['연합-살아있음']) });
      if (url.includes('edaily'))      return makeRes({ body: xmlFeed(['이데일리-살아있음']) });
      if (url.includes('coindesk'))    return makeRes({ body: xmlFeed(['coindesk-ok']) });
      if (url.includes('asiae'))       return makeRes({ status: 200, contentType: 'text/html; charset=utf-8', body: '<!DOCTYPE html><html>Just a moment…</html>' });
      return makeRes({ status: 404, body: '' });
    };
    const items = await collectIssueSourceNews(30);
    assert(!items.some(it => it.source === '아시아경제 증권'), '2: 챌린지 HTML은 실패 처리 → 아경 0건');
  ...
  // ── 3. 정상 XML: 전 피드 수집 ──
  {
    global.fetch = async (url) => {
      ...
      if (url.includes('asiae'))       return makeRes({ contentType: 'text/xml;charset=UTF-8', body: xmlFeed(['아경-A']) });
      if (url.includes('edaily'))      return makeRes({ contentType: 'text/xml;charset=UTF-8', body: xmlFeed(['이데일리-A']) });
      if (url.includes('coindesk'))    return makeRes({ body: xmlFeed(['coindesk-A']) });
      return makeRes({ status: 404, body: '' });
    };
    const items = await collectIssueSourceNews(30);
    assert(has(items, '아경-A') && has(items, '이데일리-A') && has(items, '연합-A') && has(items, 'coindesk-A'), '3: 정상 시 전 피드 수집');
  }
