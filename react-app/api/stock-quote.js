/**
 * api/stock-quote.js — Finnhub 미국 주식 시세 + 스파크라인
 *
 * GET /api/stock-quote?symbols=AAPL,MSFT,GOOGL
 *   → { items: [...], fetched_at: ISO }
 *
 * 반환 아이템 형식 (홈 카드·WatchlistCard 호환):
 *   { id, symbol, name, price, prev_close, change, change_pct, direction,
 *     sparkline: number[], category: '미국주식', source: 'Finnhub' }
 *
 * 캐시: 심볼 조합별 5분 인메모리 (CDN s-maxage=300)
 * symbols 상한: 최대 10개 (각 2 req → 20 req/batch, 60 req/min 무료 한도 내)
 * 키 미설정 시: 503 + 안내 메시지
 */
...
export default async function handler(req, res) {
  ...
  try {
    const items     = await fetchStockPrices(symbols);
    ...
    console.log(`[stock-quote] symbols=[${symbols.join(',')}] → ${items.length}개`);
    items.forEach(it => {
    ...
    return res.status(502).json({ error: 'Finnhub 시세 조회 실패', details: err.message });
  }
}
