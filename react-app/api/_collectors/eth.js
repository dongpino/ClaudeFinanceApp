/**
 * _collectors/eth.js — ETH 수집 (Vercel 서버리스 전용)
 * _collectors/btc.js와 완전히 동일한 구조 — CoinGecko(현재가/30일) + Binance(90일 OHLC,
 * 실패 시 CoinGecko OHLC 폴백)만 bitcoin→ethereum, BTCUSDT→ETHUSDT로 교체.
 */
...
async function fetchHistory90dCoinGecko() {
  const raw = await fetchJSON('https://api.coingecko.com/api/v3/coins/ethereum/ohlc?vs_currency=usd&days=30');
...
export async function collectETH({ include90d = true } = {}) {
  ...
  if (include90d) {
    try {
      ...
      console.warn(`[eth] Binance 실패: ${e.message} → CoinGecko 백업`);
      try {
        history_90d    = await fetchHistory90dCoinGecko();
        ohlc_available = true;
      } catch (e2) {
        console.warn(`[eth] CoinGecko도 실패: ${e2.message}`);
      }
    }
  }

  const item = {
    id: 'eth', name: '이더리움 ETH-USD', symbol: 'ETH-USD',
  ...
  console.log(`[eth] $${item.price.toLocaleString('en-US')}  ${sign(item.change)} (${sign(item.change_pct)}%)  hist=${history.length}  hist_90d=${history_90d.length}`);
  return item;
}
