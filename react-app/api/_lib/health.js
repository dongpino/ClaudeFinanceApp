 ...
 */
export function recordValidation(scope, { checked = 0, blocked = 0, skipped = 0, skipReasons, counters, reason, detail, fields, blockDiversity } = {}) {
  if (!scope) return;
  void persistValidation(scope, checked, blocked, reason, detail, fields, skipped, skipReasons, blockDiversity, counters);
}
...
async function persistValidation(scope, checked, blocked, reason, detail, fields, skipped = 0, skipReasons, blockDiversity, counters) {
  const r = getRedis();
  if (!r) return;
  try {
    ...
    // ── counters — 스킵이 아니어서 skip 히스토그램에 못 넣는 계측 ──────
    // ⚠️ skipped 합계를 오염시키지 않으려고 별도 접두어로 쓴다. 대표 항목은 prov:legacy —
    //    "출처 도장 없는 구버전 lastgood 스냅샷을 검사에 썼다"는 뜻이고, 검사는 시계 폴백으로
    //    **성립하므로 스킵되지 않는다.** skip 히스토그램만 보면 보이지 않는 상태라 따로 센다.
    //    배포 후 첫 성공 수집에서 전 항목이 재기록되므로 **0으로 수렴해야 정상**이다.
    for (const [ck, n] of Object.entries(counters ?? {})) p.hincrby(key, `${scope}:${sanitizeCode(ck)}`, n);
    if (blocked) {
