 ...
 *
 * ── 확인 등급 어휘(강한 순) ───────────────────────────────────────────
 *   자체실측  우리가 수집한 데이터로 값을 검증했다(거래일 캔들 유무 등). 가장 강하다 —
 *             제3자의 서술이 아니라 시장이 실제로 그렇게 움직였다는 증거이기 때문이다.
 ...
 * ⚠️ 열람했다는 사실과 출처의 권위는 다른 축이다. 벤더 API가 HTTP 200을 줬다는 것은
 ...
 *             지정하려는 것임"
 *    ③ [원문] 우주항공청 「2027년 월력요항」 발표 (천문법에 따른 달력 제작 기준)
 ...
 *      요청 https://publicholidays.co.kr/ko/2027-dates/ (2026-07-29, HTTP 200)
 *      발췌 "5월 13일|목요일|부처님 오신 날"
 *      ⚠️ 같은 표가 조문과 어긋나는 항목을 둘 갖고 있다 — 2027-06-07을 "현충일 휴일"로
 *         싣고(조문상 대체 대상 아님), 제헌절 대체(7/19)를 누락했다. 즉 이 출처는
 *         5/13을 지지하되 그 자체로는 신뢰 근거가 약하다. 제3자 표를 그대로 베끼지 말 것.
 *      ⚠️ 이 세션에서 실제로 취득한 제3자는 위 1곳뿐이다(timeanddate 403, dallyeok 403,
 *         ko.wikipedia 2027 미수록). 엄밀히는 "독립 2경로" 미달이며, 월력요항 붙임(③)이
 *         확보되면 곧바로 [원문]으로 승격해야 한다.
 *
 *  - 미국  [원문]
 ...
 */
// ⚠️ 라벨 미정(2026-07-29, 판단 대기): '근로자의날' 두 항목(2026-05-01, 2027-05-01)은
//    법률 제21543호(2026-04-09 공포, 2026-05-01 시행)로 **법정공휴일 '노동절'**이 됐다
//    (규정 제2조제6호 "노동절(5월 1일)"). 날짜는 그대로이고 라벨만 어긋난다.
//    소급 문제는 실제로 발생하지 않는다 — 표의 두 항목이 모두 시행일 당일 이후이고,
//    2026-05-01은 정확히 시행일 그 날이다. 그래도 지시 없이 바꾸지 않고 남겨 둔다.
