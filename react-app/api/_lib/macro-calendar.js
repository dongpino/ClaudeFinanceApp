 ...
 *                  "(날짜 미확정)"으로 구분 표시하고, 확정되면 날짜 갱신 + 플래그 제거한다.
 *                  ⚠️ 어휘: "잠정"은 이벤트 종류(잠정실적=가이던스)에만 쓰고 날짜
 *                  불확실성은 "날짜 미확정"으로 통일한다(2026-07-29 규약).
 *
 ...
 *
 * TODO(규약 — **캘린더 감사 범위 밖으로 이관**, 2026-07-29): region:'KR'의 의미가 정의돼
 *   있지 않다 — "발표 주체 국가"인지 "영향받는 시장"인지. MSCI는 글로벌 지수사업자
 *   (발표는 CET 23:00)라 두 해석이 갈리고, 실제로 이 파일 안에서 두 해석이 섞여 있다
 *   (MSCI='KR'은 영향 시장, FOMC/CPI='US'는 둘이 우연히 일치, 실적은 발행사 국가).
 *   ⚠️ 다만 **확산 위험은 없다**(2026-07-29 경로 조사): region을 실제로 읽는 곳은
 *      briefing-core.calendarEventLabel의 expiry 분기 하나뿐이고, 만기일의 region은
 *      두 해석이 같은 답을 준다(한국 만기일/미국 만기일). CPI는 "미국"을 하드코딩한다.
 *   → 값 오류가 아니라 규약 결정 사안이므로 정정 작업이 아니라 별도 안건으로 다룬다.
 *
 * TODO(기능 — 같은 이유로 이관): FOMC의 SEP(경제전망요약) 동반 여부가 이벤트에 노출되지
 *   않는다. 배열에 SEP 필드가 없고 주석에만 "3·6·9·12월"로 적혀 있다(연준 원문의
 *   별표 표기와 일치함은 2026-07-29 확인). 값이 틀린 게 아니라 정보가 없는 것이라
 *   정정이 아니라 기능 추가 안건이다.
 */
 ...
 *
 * ── 분기 명칭 표기 규약(2026-07-29) ─────────────────────────────
 * **회계연도 기업의 분기 명칭은 각 사 공식 표기를 보존하고, title 끝에 "(회계연도)"만 붙인다.**
 *   · 애플 FY26 3분기(2026-07-30 발표), 엔비디아 FY27 2분기(2026-08-26 발표) — 회계연도가
 *     역년과 어긋나 엔비디아는 2026년 발표가 FY27이다. 각 사 공식 명칭이자 Finnhub 응답의
 *     quarter/year와도 일치하므로(AAPL Q3/2026, NVDA Q2/2027) 바꾸지 않는다.
 *   · 삼성전자는 회계연도 = 역년이라 괄호를 붙이지 않는다(2Q26 그대로).
 * ⚠️ **대상 기간(예: 2026년 5~7월)은 병기하지 않는다.** Finnhub 페이로드에 기간 필드가 없어
 *    손으로 도출해야 하고, 실적은 이 파일에서 오류 이력이 가장 많고 근거 등급이 가장 약한
 *    ([벤더확인]) 카테고리다 — 수동으로 채워야 하는 필드를 늘리지 않는다.
 * ⚠️ shortLabel은 건드리지 않았다("애플 실적"). Haiku 프롬프트로 가는 건 shortLabel뿐이라
 *    이 변경은 UI title에만 영향한다(프롬프트 토큰 영향 0).
 *
...
export const EARNINGS_EVENTS_2026 = [
  ...
  { date: '2026-07-30', title: '애플 FY26 3분기 실적 발표 (회계연도)',   shortLabel: '애플 실적',   category: 'earnings', region: 'US' },
  { date: '2026-08-26', title: '엔비디아 FY27 2분기 실적 발표 (회계연도)', shortLabel: '엔비디아 실적', category: 'earnings', region: 'US' },
  ...
  { date: '2026-10-28', title: '애플 FY26 4분기 실적 발표 (회계연도)',   shortLabel: '애플 실적',   category: 'earnings', region: 'US', tentative: true },
  ...
  { date: '2026-11-17', title: '엔비디아 FY27 3분기 실적 발표 (회계연도)', shortLabel: '엔비디아 실적', category: 'earnings', region: 'US', tentative: true },
];
 ...
 * 전휴장했는데 앱은 그날을 미국 위칭데이로 표시했다(2026-07-28 감사에서
 ...
 *    ①-2 [원문 · 자동] **KASI 특일정보 자동대조(일 1회 크론)** — 2026-07-29 도입
 *         api/holiday-audit-cron.js가 매일 06:20 KST에 SpcdeInfoService/getRestDeInfo를
 *         연 단위로 호출해 이 표와 날짜 기준으로 대조하고 결과를 상태판에 노출한다.
 *         · 도입 시점 실측: 누락 0 / 오탑재 0 / KRX 고유 2(12-31) / 일치 46 / 명칭차 24(참고)
 *         · **자체실측이 못 보는 미래 구간을 이쪽이 본다.** depletion은 "표가 마르는 것"만
 *           잡고 "제도가 바뀌는 것"은 못 잡는데(커버리지 수평선이 안 움직인다), 이번 제헌절
 *           누락 4건이 정확히 그 사각이었다. 판정·화이트리스트는 _lib/holiday-audit.js.
 *         ⚠️ KASI도 유한하다(현재연도+2년, 2029는 0건). 우리 표가 더 멀리 가면 그 연도는
 *            "자동검증 불가"로 표시된다 — 통과가 아니라 미검증이다.
 ...
 *
 *  - 2027 부처님오신날 5/13  [원문]  ← 2026-07-29 [교차확인]에서 승격
 *      요청 https://apis.data.go.kr/B090041/openapi/service/SpcdeInfoService/getRestDeInfo
 *           ?solYear=2027&_type=json&numOfRows=100  (2026-07-29, HTTP 200, resultCode=00)
 *      발췌 {"dateKind":"01","dateName":"부처님오신날","isHoliday":"Y","locdate":20270513}
 *      비고 KASI(한국천문연구원)는 월력요항을 산출하는 발표 주체이고 이 API가 그 데이터의
 *           공식 배포 경로다(공공데이터포털 SpcdeInfoService) → [원문] 등급이 성립한다.
 *           취득 방식: Claude Code가 KASI_API_KEY로 **직접 호출**해 응답을 받았다
 *           (아래 옛 근거들과 달리 요청 URL·상태·발췌가 모두 우리 기록에 있다).
 *      ⚠️ 아래는 승격 전 근거다 — 이력으로만 남긴다.
 *      (구)주근거 ① 국제뉴스(2026-05-24)
 ...
 *  미국: 3·6·9·12월 셋째 금요일(트리플 위칭)만 해당 — 미국은 매월이 아님.
 *
 * ── 한미 용어 비대칭(2026-07-29 정정) ─────────────────────────
 * ⚠️ **양쪽을 같은 용어로 통일하지 말 것.** 만기하는 상품 종류가 실제로 다르다.
 *   · 미국 [원문] 2020년 9월 OneChicago(마지막 개별주식선물 거래소) 폐쇄로 개별주식선물이
 *     사라져 **3종**(지수선물·지수옵션·개별주식옵션)만 남았다 → "트리플 위칭"이 사실이다.
 *     Nasdaq "The Powerful Impact of Triple Witching"(nasdaq.com):
 *       "with the close of OneChicago in September 2020, single stock futures ceased
 *        trading, so the day is now just a triple witching"
 *     ⚠️ 취득 경로: **Claude 대화 세션 검색으로 취득(2026-07-29)**, Claude Code 직접
 *        fetch가 아니다 — 요청 URL·응답 상태를 우리가 기록하지 못했다.
 *   · 한국은 KRX에 개별주식선물이 **존속**하므로 4종 만기가 그대로 사실이다 →
 *     "네 마녀의 날" 유지가 맞다.
 * ⚠️ 다수 매체가 미국에 대해서도 "쿼드러플 위칭"을 관행적으로 계속 쓴다(용어가 혼용된다).
 *    그래서 외부 기사와 대조할 때 우리 표기가 틀린 것처럼 보일 수 있는데, 상품 구성으로
 *    따지면 트리플이 맞다. 되돌리기 전에 이 주석을 먼저 읽을 것.
 * @param {number} year
 */
