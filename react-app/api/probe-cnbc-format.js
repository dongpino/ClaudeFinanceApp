 ...
 *
 * ⚠️ 은퇴 조건 — 조사 장치다. 형식 게이트 판정이 끝나고, **2026-09-14 애프터마켓 시행 후
 *    신규 토큰 캡처까지 완료되면** 이 크론(vercel.json 항목 포함)을 삭제한다.
 *    probe-daum-status.js/probe-backup.js와 **동시 은퇴**하며, Redis 키는 마지막 쓰기
 *    +7일에 스스로 사라진다(probe-store.js TTL).
 *    ※ 이 프로브 자체는 미국 장을 보지만, 은퇴는 세 조사 장치를 한 번에 걷어내는 편이
 *      낫다(따로 남기면 잊힌다). KR 거래시간 변경이 트리거가 되는 이유가 그것이다.
 *
